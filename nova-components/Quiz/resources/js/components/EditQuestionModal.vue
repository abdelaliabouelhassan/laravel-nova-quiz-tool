            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 space-y-5">
                ...
                <div class=" w-full p-5 space-y-6">
                    <div class=" w-full space-y-4">
                        ...
                        <input v-model="question.title" type="text" id="Question" class=" outline-none bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Question...?" required>
                    </div>
                    <div class="flex items-center mb-4">
                              <input @change="setComment" :checked='isCommentChecked' id="Comment" type="checkbox" name="comment" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                             <label for="Comment" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Has Comment?</label>
                    </div>
                    <div class="flex items-center mb-4">
                            <input @change="setGraded" :checked='isGradedChecked'  id="Graded" type="checkbox" name="grade"  class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    ...
                    <div class=" w-full flex flex-col items-start space-y-5">
                        <div class=" flex items-center space-x-4 w-full" v-for="(item,index,key) in question.answers" :key="key">
                            <div class=" w-full max-w-lg">
                                <input v-model="item.question" type="text"  class="  outline-none bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" :placeholder="'Answer ' + (index + 1)" required> 
                            </div>
                            <div class=" w-fit flex items-center space-x-2">
                                <input v-model="item.is_correct" @change="SetIsCorrect(item)" :checked='item.is_correct' :value="true" type="radio"  name="qs" class="w-6 h-6 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
...
<script setup>
 ...
 const isGradedChecked = computed(() => {
 ...
 const SetIsCorrect = (item) => {
  ...
  props.question.answers.forEach((element,index) => {
    if(element != item){
  ...
  const Save = () => {
    ...
    let is_correct = props.question.answers.find(item => item.is_correct == true)
    ...
    let has_answer = props.question.answers.find(item => item.question == '')
    if(has_answer){
  ...
  const RemoveAnswer = (item) => {
    let index = props.question.answers.indexOf(item)
